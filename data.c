...
int createFile()
{
    ...
    if (info->tm_mon < 9)
    {
        sprintf(filepath, "%s/%s_%d-0%d.csv", data_path, site_name, info->tm_year + 1900, info->tm_mon + 1);
    }
    else
    {
        sprintf(filepath, "%s/%s_%d-%d.csv", data_path, site_name, info->tm_year + 1900, info->tm_mon + 1);
    }
